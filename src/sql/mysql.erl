...
start_link(PoolId, Host, undefined, User, Password, Database, Encoding) ->
	start_ok(PoolId, Host, ?PORT, User, Password, Database, Encoding, start_link);
...
start(PoolId, Host, undefined, User, Password, Database, Encoding) ->
	start_ok(PoolId, Host, ?PORT, User, Password, Database, Encoding, start);
start(PoolId, Host, Port, User, Password, Database, Encoding) ->
	start_ok(PoolId, Host, Port, User, Password, Database, Encoding, start).

start_ok(PoolId, Host, Port, User, Password, Database, Encoding, StartFunc) ->
	crypto:start(),
	gen_server:StartFunc({local, ?SERVER}
		, ?MODULE
...
connect(PoolId, Host, Port, User, Password, Database, Encoding, Reconnect, LinkConnection) ->
	...
	Fun = if LinkConnection ->
			fun mysql_conn:start_link/7;
		true ->
			fun mysql_conn:start/7
		end,
...
init([PoolId, Host, Port, User, Password, Database, Encoding]) ->
	case mysql_conn:start_link(Host, Port, User, Password, Database, Encoding, PoolId) of
		...
		{error, _Reason} ->
			?ERRLOG(["failed starting first MySQL connection handler exiting"]),
...
handle_call({add_conn, Conn}, _From, State) ->
	...
	{_PoolId, _ConnPid} = {Conn#conn.pool_id, Conn#conn.pid},
	%%  ?ERRLOG(["added connection to my list | [PoolId, ConnPid] => ", [PoolId, ConnPid]]),
	{reply, ok, NewState}.
...
handle_cast({prepare, Name, Stmt}, State) ->
	...
	?ERRLOG(["received prepare/2: [Name, Version1, Stmt] => ", [Name, Version1, Stmt]]),
	{noreply, State#state{prepares = gb_trees:enter(Name, {Stmt, Version1}, State#state.prepares)}};

handle_cast({unprepare, Name}, State) ->
	?ERRLOG(["received unprepare/1: [Name] => ", [Name]]),
	State1 = case gb_trees:lookup(Name, State#state.prepares) of
		none ->
			?ERRLOG(["trying to unprepare a non-existing "]),
			State;
		{value, _Stmt} ->
			State#state{prepares = gb_trees:delete(Name, State#state.prepares)}
	end,
	{noreply, State1}.
...
handle_info({'DOWN', _MonitorRef, process, Pid, Info}, State) ->
	...
	{ok, Conn, NewState} ->
		?ERRLOG(["connection exited: [Pid, Info] => ", [Pid, Info]]),
		...
		{noreply, NewState};
	error ->
		?ERRLOG(["received 'DOWN' signal from pid not in my list: [Pid] => ", [Pid]]),
		{noreply, State}
	end.

terminate(Reason, _State) ->
	?ERRLOG(["terminating with reason:: [Reason] => ", [Reason]]),
	Reason.
...
start_reconnect(Conn) ->
	...
	?ERRLOG(["started pid to try and reconnect to PoolId, Host, Port (replacing connection with oldpid): [pid, PoolId, Host, Port, oldpid] => "
...
reconnect_loop(Conn, N) ->
    ...
    case connect(PoolId,
        ...
        {ok, ConnPid} ->
            ?ERRLOG(["managed to reconnect to PoolId:Host:Port (connection ConnPid) : [PoolId, Host, Port, ConnPid] => ", [PoolId, Host, Port, ConnPid]]),
            ok;
        {error, Reason} ->
            NewN = case N of
                1 ->
                    ?ERRLOG(["reconnect: still unable to connect to : [PoolId, Host, Port, Reason] => ", [PoolId, Host, Port, Reason]]),
                    0;
                _ ->
                    N + 1
            end,
            timer:sleep(5000),
            reconnect_loop(Conn, NewN)
    end.
